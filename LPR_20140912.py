...
if maxima_calc==1:
   str = 'Frequency Mod Maxima Max (PPM) (100MHz), ' + \
         '%.2f' % ((lpr1.maxima_max-100*MHz)/(100*MHz)*MHz)
   resultFile.write(str + '\n')
   print(str)

   str = 'Frequency Mod Maxima Mean (PPM) (100MHz), ' + \
         '%.2f' % ((lpr1.maxima_mean-100*MHz)/(100*MHz)*MHz)
   resultFile.write(str + '\n')
   print(str)

   str = 'Frequency Mod Maxima Min (PPM) (100MHz), ' + \
         '%.2f' % ((lpr1.maxima_min-100*MHz)/(100*MHz)*MHz)
   resultFile.write(str + '\n')
   print(str)
...
for spec in ECspecList:
    if spec == PCIe_1:
         str = spec.name + ' : pk-pk (ps) Spec = 86 ps,' + '%3.3f' % (spec.p2p/ps)
         resultFile.write(str + '\n')
         print(str)
         str = 'PCIe Gen 2 WORST RMS loBand (ps) Spec = 3 ps, ' + \
         '%.3f' % (PCIe_2_WORST_rms_loBand/ps)
         resultFile.write(str + '\n')
         print(str)

         str = 'PCIe Gen 2 WORST RMS hiBand (ps) Spec = 3.1 ps, ' + \
               '%.3f' % (PCIe_2_WORST_rms_hiBand/ps)
         resultFile.write(str + '\n')
         print(str)

         str = PCIe_2_SRIS.name + ' : RMS   (ps) Spec = 3 ps,' + '%3.3f' % (PCIe_2_SRIS_WORST_rms/ps)
         resultFile.write(str + '\n')
         print(str)

         str = 'PCIe Gen 3 WORST RMS(ps) Spec = 1 ps, ' + \
               '%.3f' % (Gen3_PCIe_3_WORST_rms/ps)
         resultFile.write(str + '\n')
         print(str)

         str = PCIe_3_SRIS.name + ' : RMS   (ps) Spec = 0.5 ps,' + '%3.3f' % (PCIe_3_SRIS_WORST_rms/ps)
         resultFile.write(str + '\n')
         print(str)
...
if spectrum_plot:
   ...
   absspectrum = abs(PCIe_2A_COMMON_Low.filteredPhaseSpectrum) *2  #/1e12*1e0
