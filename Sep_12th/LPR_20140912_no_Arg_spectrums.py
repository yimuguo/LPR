...
##maxima_calc=sys.argv[3]
##minima_calc=sys.argv[3]
maxima_calc=1
minima_calc=1
#--------------------------------------------
...
for spec in ECspecList:
    if spec == PCIe_1:
         ...
         resultFile.write(str + '\n')
         print(str)
    str = 'PCIe Gen 2 WORST RMS loBand (ps) Spec = 3 ps, ' + '%.3f' % (PCIe_2_WORST_rms_loBand/ps)
    resultFile.write(str + '\n')
    print(str)

    str = 'PCIe Gen 2 WORST RMS hiBand (ps) Spec = 3.1 ps, ' +  '%.3f' % (PCIe_2_WORST_rms_hiBand/ps)
    resultFile.write(str + '\n')
    print(str)

    str = PCIe_2_SRIS.name + ' : RMS   (ps) Spec = 3 ps, ' + '%3.3f' % (spec.rms/ps)
    resultFile.write(str + '\n')
    print(str)

    str = 'PCIe Gen 3 WORST RMS(ps) Spec = 1 ps, ' + '%.3f' % (Gen3_PCIe_3_WORST_rms/ps)
    resultFile.write(str + '\n')
    print(str)

    str = PCIe_3_SRIS.name + ' : RMS   (ps) Spec = 0.5 ps,' + '%3.3f' % (spec.rms/ps)
    resultFile.write(str + '\n')
    print(str)
...
if spectrum_plot:
   ...
   absspectrum = abs(windowedPhaseSpectrum) *2  #/1e12*1e0
